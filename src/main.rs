...
use noir_sidecar::{db::index::sqlite::SqliteAccountsIndex, rpc::JsonRpcModule};
use std::sync::Arc;
#[cfg(not(feature = "mock"))]
use {
    noir_sidecar::{
        client::Client,
        db::index::{postgres::PostgresAccountsIndex, AccountsIndex},
...
use {
    noir_sidecar::{
        db::index::traits::AccountsIndex as _,
        rpc::solana::mock::svm::{LiteSVM, SvmRequest},
    },
    tokio::signal,
};
...
async fn main() -> anyhow::Result<()> {
    ...
    #[cfg(feature = "mock")]
    {
        let accounts_index = Arc::new(SqliteAccountsIndex::create(config.sqlite).unwrap());
        accounts_index
            .create_index()
            .await
            .expect("Failed to initialize AccountsIndex.");
        let (svm, svm_rx) = LiteSVM::new(accounts_index.clone());
        let module = JsonRpcModule::create(svm.tx.clone(), accounts_index.clone())
            .map(Arc::new)
            .expect("Failed to create jsonrpc handler.");

        let svm_tx = svm.tx.clone();
